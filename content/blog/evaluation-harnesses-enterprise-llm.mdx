...
**Count:** ~800 tests  
**What they verify:** JITNA handshakes, DelentiaDB write-read cycles, HexaCore routing logic
...
**Count:** ~200 tests  
**Scenarios:** Model provider outage, network partition, DelentiaDB hot zone full, SignedAI consensus deadlock  
...
Every core system has mathematical invariants that must hold unconditionally. For FDIA: the Architect gate invariant. For SignedAI: the consensus threshold invariant. For DelentiaDB: the PDPA erasure invariant. These are implemented as property-based tests (Level 8) using Hypothesis — not as hand-written examples.
...
Without contract tests, a change to the DelentiaDB query format breaks the Delta Engine silently. With contract tests, the CI pipeline rejects the change at the boundary.
...
A: Level 6 security tests include dedicated PDPA compliance tests: (1) write a record → (2) request erasure → (3) verify UUID tombstone → (4) verify no retrievable data. This runs on every commit that touches DelentiaDB.
