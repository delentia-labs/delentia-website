...
The Regional Language Adapter in `rct-platform` sits between user input and the FDIA scoring pipeline. It operates in three stages:
...
The Regional Language Adapter covers ASEAN-focused language support in the current public SDK line, with the exact verified checkpoint maintained separately in `docs/testing/TESTING_CANONICAL.md`:
...
At the current public SDK checkpoint, the canonical proof for tests, coverage, and version lives in `docs/testing/TESTING_CANONICAL.md`. The adapter-specific measurements below should be read as module-level evaluation notes:
...
This is why the adapter is one of the five core modules in `rct-platform` — not an optional add-on, but a prerequisite for trustworthy AI in the Thai market.

---

## Getting Started

The Regional Language Adapter is available in the Apache-2.0 `rct-platform` SDK line. For the current verified package checkpoint, refer to `docs/testing/TESTING_CANONICAL.md`:
...
Full documentation is available at the [RCT Platform SDK docs](https://rctlabs.github.io/rct-platform/).
