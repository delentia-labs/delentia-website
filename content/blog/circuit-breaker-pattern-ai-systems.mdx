...
**Signed failure records.** When a circuit breaker opens, the RCT Platform generates a signed failure record in DelentiaDB. This record contains the component identity, the failure signal, the FDIA scores at the time of failure, and the timestamp. Engineers can reconstruct exactly why a circuit opened and what the quality signal looked like before it opened.
